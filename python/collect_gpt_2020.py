...
DEMOGRAPHICS_FILE = "demographics/demographics_2020_T1LC.txt" # Need to be changed considering the group (T1CL vs. T2CL)
QUESTIONS_FILE = "questions/questions_2020_T1LC.txt" # Need to be changed considering the group (T1CL vs. T2CL)
...
def generate_intro_prompt(participant):
    return (
        ...
        "You are participating in a survey conducted just before the 2020 U.S. presidential election.\n"
        "You will be asked about how likely you think it is that one of the two candidates will win the popular vote in various U.S. states.\n"
        ...
        "For each question, respond with a number between 0 and 100.\n"
        "Do not rely on knowledge of the actual outcomes—respond as if you are answering in real time before the results were known."
    )

def generate_question_prompt(question):
    return (
        f"Question: {question}\n"
        "Answer with a single number between 0 and 100. Do not include any additional text."
    )
...
def run_experiment():
    ...
    output_file = os.path.join(OUTPUT_DIR, f"results_2020_{model_tag}_{timestamp}.csv")
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for participant in participants:
            ...
            if intro_response is None:
                print(f"Intro prompt failed for {participant['id']}.")
                continue
